...
dataset = torch.load("D:\\桌面\\materials code\\cif2graph_data\\dataset\\MP_train_dataset_Ef.pth")
dataset_v = torch.load("D:\\桌面\\materials code\\cif2graph_data\\dataset\\MP_valid_dataset_Ef.pth")
...
def ray_train(config):
    model = Is_Stable_GNN(train_dataset.num_node_features, train_dataset.num_edge_features)
...
def valid(model, epoch):
    ...
    return valid_average_loss, valid_losses_whole, r2_average, r2_whole
...
if __name__ == '__main__':
    ...
    valid_loader = DataLoader(dataset=valid_dataset, batch_size=1000, shuffle=True)
    # test_loader = DataLoader(dataset=test_dataset, batch_size=2500, shuffle=True)
    model = Is_Stable_GNN(train_dataset.num_node_features, train_dataset.num_edge_features, layer_nums=4, l1_num=4,
                          l2_num=4)
    model = model.to(device)
    # ----------------------------训练----------------------------------------

    # test_average_loss, test_losses_whole, r2_average, r2_whole = test(model)
    ...
    scheduler = MultiStepLR(optimizer, milestones=[100, 200, 300, 400, 500], gamma=0.1)
