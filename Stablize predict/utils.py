...
supercell_matrix_list = [[1, 1, 1], [1, 1, 2], [1, 2, 1], [1, 2, 2], [2, 1, 1], [2, 1, 2], [2, 2, 1], [2, 2, 2],
                         [1, 3, 3], [3, 3, 3]]
...
def build_dict():
    write_file = codecs.open("D:\\桌面\\materials_code\\cif2graph_data\\label\\Ef_dic.txt", "w", "utf_8")
    listdir_json = os.listdir("D:\\桌面\\materials_code\\cif2graph_data\\json\\")
    for filename in listdir_json:
        print(filename)
        whole_json = loadfn("D:\\桌面\\materials_code\\cif2graph_data\\json\\" + filename)
...
def muti_process(listdir_cif, cnn, filename, lable_dic):
    ...
    datalist = []
    #     break
    parser = CifParser("D:\\桌面\\materials_code\\cif2graph_data\\cif\\" + filename)
    structure_origin = parser.parse_structures(primitive=False)[0]
    for supercell_matrix in supercell_matrix_list:
        structure = structure_origin.copy()
        ...
        data = Data(x=x,
                    edge_index=torch.tensor(edge_index, dtype=torch.long).t().contiguous(),
                    edge_attr=torch.tensor(edge_attr, dtype=torch.float), y=torch.tensor(y, dtype=torch.float),
                    cif=filename)
...
def build_dataset():
    ...
    lable_dic = {}
    read_file = codecs.open("D:\\桌面\\materials_code\\cif2graph_data\\label\\Ef_dic.txt", "r", "utf_8")
    listdir_json = os.listdir("D:\\桌面\\materials_code\\cif2graph_data\\json\\")
    ...
    listdir_cif = os.listdir("D:\\桌面\\materials_code\\cif2graph_data\\cif\\")
    output_cifs = []
    for filename in listdir_cif:
    ...
    torch.save(dataset, "D:\\桌面\\materials_code\\cif2graph_data\\dataset\\MP_dataset_whole_Ef.pth")
